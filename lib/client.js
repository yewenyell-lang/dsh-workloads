...
function WorkloadCard({item,selected,onSelect,onAction,busy}){const active=["starting","running","stopping"].includes(item.phase);return h("div",{role:"button",tabIndex:0,onClick:()=>onSelect(item.workloadId),onKeyDown:(e)=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();onSelect(item.workloadId)}},style:{width:"100%",boxSizing:"border-box",textAlign:"left",cursor:"pointer",borderRadius:10,border:"1px solid "+(selected?C.brand:C.b1),background:selected?C.l2:C.l1,color:C.p,padding:"12px 14px",display:"grid",gap:7}},
  ...
  h("div",{style:{display:"flex",gap:8,marginTop:2}},
    item.capabilities&&item.capabilities.stop?h("button",{type:"button",disabled:Boolean(busy),onClick:(e)=>{e.stopPropagation();onAction("stop",item.workloadId)},style:{padding:"5px 9px",border:"1px solid "+C.b2,borderRadius:7,background:"transparent",cursor:"pointer",color:C.warn,opacity:busy?.endsWith(item.workloadId)?.5:1}},"停止"):null,
...
function apply(ctx){
  ...
  function RuntimeCenter({useSessions,sessionId}){
    ...
    const [snapshot,setSnapshot]=React.useState(null),[error,setError]=React.useState(""),[selected,setSelected]=React.useState(""),[logs,setLogs]=React.useState(null),[revision,setRevision]=React.useState(0),[busy,setBusy]=React.useState("");
    ...
    const action=async(op,workloadId)=>{if(busy)return;const key=op+":"+(workloadId||"all");setBusy(key);setError("");try{if(op==="stopAll"){try{await post(op,cwd,sessionId)}catch(e){if(!(e instanceof Error)||!e.message.includes("unknown operation"))throw e;for(const item of rows.filter((x)=>["starting","running","stopping"].includes(x.phase)))await post("stop",cwd,sessionId,item.workloadId)}}else await post(op,cwd,sessionId,workloadId);setRevision((v)=>v+1)}catch(e){setError(e instanceof Error?e.message:String(e))}finally{setBusy("")}};
